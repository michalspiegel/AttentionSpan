...
import math
from models.normal_multihead_attn import MultiheadAttention
...
class MultiHeadDifferentialAttention(nn.Module):
    ...
    def forward(self, X):
        ...
        Args:
            X (Tensor): Input tensor of shape (batch, sequence_length, d_model).
            attention_labels (Tensor): boolean tensor (corresponding to the input tensor X) marking significant tokens 
        ...
        attn_clone = attention.clone().detach()
...
class DiffTransformerLayer(nn.Module):
    ...
    def __init__(self, d_model, num_heads, lambda_init):
        ...
        #self.attn = MultiheadAttention(d_model, num_heads, dropout=0.0, batch_first=True, bias=True)
    ...
    def forward(self, x):
        ...
        x = self.norm1(x)
        #y, attn_scores = self.attn(x,x,x, need_weights=True, average_attn_weights=False, is_causal=True)
        y, attn_scores = self.attn(x)
        y = y + x
...
class DiffTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            attn_scores_per_layer.append(attn_scores)
        
        X = self.norm(X)  # (batch, N, d_model)
        logits = self.head(X)  # (batch, N, vocab_size)
        return logits, torch.stack(attn_scores_per_layer, dim=1)
